...
void DatabaseManager::insertCsvRecord(QString fileOneLine, QString datasetId)
{
    ...
    sqlQuery.prepare(
                ...
                "(`dataset_id`, `Data file name`, `Scan(s)`, `Proteoform ID`, `Protein accession`, `Protein description`, `Proteoform`,`Fragmentation`) "
                "VALUES (:dataset_id, :Data_file_name, :Scans, :Proteoform_ID, :Protein_accession, :Protein_description, :Proteoform,:Fragmentation)"
            );
    sqlQuery.bindValue(":dataset_id", datasetId);
    sqlQuery.bindValue(":Data_file_name", oneLineColumns.at(0));

//    qDebug() << "Data_file_name " <<  oneLineColumns.at(0) <<  endl;

    sqlQuery.bindValue(":Scans", oneLineColumns.at(4).toInt());

//    qDebug() << "Scans " <<  oneLineColumns.at(4) <<  endl;

    sqlQuery.bindValue(":Proteoform_ID", oneLineColumns.at(10).toInt());

    qDebug() << ":Proteoform_ID " <<  oneLineColumns.at(10) <<  endl;

    sqlQuery.bindValue(":Protein_accession", oneLineColumns.at(12).split('|',QString::SkipEmptyParts).last());

//    qDebug() << "Protein_accession " <<  oneLineColumns.at(12).split('|',QString::SkipEmptyParts).last() <<  endl;

    sqlQuery.bindValue(":Protein_description", oneLineColumns.at(13));

//    qDebug() << ":Protein_description " <<  oneLineColumns.at(13) <<  endl;

    sqlQuery.bindValue(":Proteoform", oneLineColumns.at(16));

    //wz add
    sqlQuery.bindValue(":Fragmentation",oneLineColumns.at(3));   //添加Framentation 字段
